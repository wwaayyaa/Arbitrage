...
(async () => {
    ...
    let tradeETH = web3.utils.toWei("5000", 'ether');
    try {
        await uniRoute2.methods
            .swapExactETHForTokens(0, [CC.token.weth.address, CC.token.dai.address], acc.address, timestamp + 300)
            .send({from: acc.address, value: tradeETH, gas: 5000000})
    } catch (e) {
        c('uni buy token', e)
    }
    c("花5000ETH在uniswap买DAI后余额: ");
    c("eth: " + utils.fromWei(await web3.eth.getBalance(acc.address)));
    c("dai: " + utils.fromWei(await dai.methods.balanceOf(acc.address).call()));
